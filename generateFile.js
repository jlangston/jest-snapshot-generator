...
const generateFile = (filePath, filename, startPath) => {
  return new Promise((resolve, reject) => {
    checkIfReactFile(filePath).then(({ isReactFile, filePath }) => {
      if (isReactFile) {
        const testPath = `${startPath}/__test__`;
  ...
  return new Promise((resolve, reject) => {
    fs.readFile(filePath, function(err1, data) {
      if (err1) {
        ...
        const res = { filePath, isReactFile: isReactFile(data) };
        resolve(res);
      }
    });
  });
...
function isReactFile(data) {
  ...
  if (
    data.indexOf(`require('react')`) >= 0 || data.indexOf(`from 'react'`) >= 0
  ) {
    return true;
  } else {
    return false;
